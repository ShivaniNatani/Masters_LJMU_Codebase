...
logger = setup_logger("evaluate_iu_real")
...
def run_condition_eval(
    ...
    use_slg: bool,
    top_k: int = 2,
    max_samples: int = 10**9,  # Evaluate first 200 real test cases for fast benchmark report
):
    predictions = []
    references = []
    sample_logs = []
    ...
    tag = f"iu_mode_{retrieval_mode}_slg_{use_slg}"
...
def main():
    logger.info("==================================================")
    logger.info("  EXCLUSIVELY EVALUATING REAL IU CHEST X-RAY DATA ")
    logger.info("==================================================")
    ...
    # Load Processed IU Chest X-Ray Data
    splits_csv = "data/processed/iu_chest_xray/iu_cxr_splits.csv"
    ...
    logger.info(f"Loaded Real IU Chest X-Ray Split: {len(train_df)} train, {len(test_df)} test samples.")
    ...
    ckpt_path = os.environ.get("EVAL_CKPT", "checkpoints/iu_real/baseline_best_loss.pt")
    ...
    # Initialize Retriever and load built FAISS index
    retriever = MultimodalRetriever(dimension=512)
    faiss_index_path = "retrieval/index_store/iu_cxr_faiss.index"
    meta_path = "retrieval/index_store/iu_cxr_metadata.json"
    ...
    if os.path.exists(faiss_index_path) and os.path.exists(meta_path):
        ...
        logger.info(f"Successfully loaded pre-built FAISS index ({retriever.vector_index.index.ntotal} vectors)")
    ...
    # Save JSON and Markdown tables
    json_path = "results/iu_real_4way_metrics.json"
    md_path = "results/iu_real_metrics_table.md"
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# Real IU Chest X-Ray 4-Way Benchmark Evaluation Matrix\n\n")
    ...
    pd.DataFrame(pred_rows).to_csv("results/iu_real_sample_predictions.csv", index=False)
    logger.info(f"Saved {len(pred_rows)} per-sample predictions to results/iu_real_sample_predictions.csv")

    logger.info("==================================================")
    logger.info(f"  REAL DATASET EVALUATION COMPLETE!")
